...
class DriverTest : Assert() {
    ...
    fun asDriver_switchMap() {
        val observable = scheduler.createColdObservable(
            next(0, 0),
            next(1, 1),
            error(2, Error("Test")),
            complete(3)
        )
        val observable1 = scheduler.createColdObservable(
            next(0, 1),
            next(0, 2),
            error(0, Error("Test"))
        )
        val observable2 = scheduler.createColdObservable(
            next(0, 10),
            next(0, 11),
            error(0, Error("Test"))
        )
    ...
    @Test
    fun asDriver_merge() {
        val observable: Observable<Int> = scheduler.createColdObservable(
            next(0, 1),
            next(0, 2),
            error(0, Error("Test"))
        )
